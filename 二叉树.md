...
removeNode 第一次调用的时候，传入的是根节点；removeNode 函数的执行流程如下：
1. 设置当前节点为根节点
2. 判断node 是不是空树
3. 当要删除节点的值小于当前节点的值,通过 node.left = removeNode(node.left, data) 重写当前节点的左节点
4. 当要删除节点的值大于当前节点的值,通过 node.right = removeNode(node.right, data) 重写当前节点的右节点
5. 当要删除的节点值等于当前节点的值
	5.1 若是叶子节点,则直接return null, 那么之后通过当前节点父节点就访问不到该元素了
	5.2 若没有左子树,则直接返回右子树,用当前节点的右子树替换当前节点,则当前节点就被删除了
	5.3 若没有右子树,则直接返回左子树,用当前节点的左子树替换当前节点,则当前节点就被删除了
	5.4 若当前节点左右子树都有,先通过 getMin(node.right) 获取右子树上的最小值(其实也可以查找左子树上的最大值),
		将当前节点替换成右子树上最小的节点,然后用 removeNode() 递归的方式在以当前元素为根节点的子树中删除右子树最小值所对应的节点
